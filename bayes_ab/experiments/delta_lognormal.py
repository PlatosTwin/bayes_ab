...
from bayes_ab.metrics import eval_delta_lognormal_agg
...
class DeltaLognormalDataTest(BaseDataTest):
    """
    Class for Bayesian A/B test.js for delta-lognormal data (log-normal with possible zeros).
    ...
    def _eval_simulation(self, sim_count: int = 20000, seed: int = None) -> Tuple[dict, dict]:
        ...
        """
        pbbs, loss, samples = eval_delta_lognormal_agg(
            self.totals,
            self.positives,
            self.sum_logs,
            self.sum_logs_2,
    ...
    def add_variant_data_agg(
        ...
        sum_logs: float,
        sum_logs_2: float,
        a_prior_beta: Number = 1,
        b_prior_beta: Number = 1,
        m_prior: Number = 1,
        a_prior_ig: Number = 0,
        b_prior_ig: Number = 0,
        w_prior: Number = 0.01,
        ...
        """
        Add variant data to test.js class using aggregated delta-lognormal data.
    ...
    def add_variant_data(
        ...
        """
        Add variant data to test.js class using raw delta-lognormal data.
